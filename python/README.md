...
|---|---|---|
| **GitHub** | `GITHUB_TOKEN` présent dans `.env` | URL stable `raw.githubusercontent.com/…` (dossier `images/` du dépôt) |
...
> ⚠️ Le dépôt est **public** : la stratégie GitHub publie les images pour tout le
> monde. Pour des documents contenant des informations sensibles (écoutes client,
> par exemple), utiliser la stratégie base64.
...
Copiez `.env.example` en `.env` (à la racine du dépôt) et renseignez vos clés :
...
```powershell
python python/sync.py 'https://docs.numerique.gouv.fr/docs/<uuid>/'
# ou avec l'UUID seul :
python python/sync.py '<uuid>'
```

### Notebook de test (debug pas-à-pas)

Ouvrir `python/test_sync.ipynb` et exécuter les cellules dans l'ordre :
